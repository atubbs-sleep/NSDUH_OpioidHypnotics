...
# First, get coefficients and confidence intervals from the models
zdrug.m.sum <- tibble()
for(idx in 1:length(outcomes)){
  for(jdx in 1:length(predictors)){
    zdrug.m.sum <- bind_rows(zdrug.m.sum,
                             bind_cols("Outcome" = outcomes[idx],
                                       "Model" = modelnames[jdx],
                                       tidy(zdrug.m[[idx]][[jdx]],exponentiate = T,conf.int = T) %>%
                                         filter(term == "PYROpioid21" | term == "PYROpioid22")
                                       )
                             )  
  }
}
zdrug.m.sum$term <- factor(zdrug.m.sum$term, levels = c("PYROpioid21","PYROpioid22"), labels = c("Opioid use","Opioid misuse"))
zdrug.m.sum$Outcome <- factor(zdrug.m.sum$Outcome, levels = c("PYRZdrug","PYRZdrugMU","SedBenzo"), 
                              labels = c("Z-drug use","Z-drug misuse","Sedative benzodiazepine use"))
# Build plot
ggbarplot(zdrug.m.sum, x="Outcome",y="estimate",fill="Model",position=position_dodge(width=1),palette = "Greys") + 
  geom_errorbar(aes(ymin=conf.low, ymax=conf.high,group=Model),position = "dodge",width=1,size=.1) +
...
# Outcome 2: PYRZdrug predicted by Covariates only
# Select data 
zdrug.op2 <- zdrug.data %>% 
  dplyr::select(PYRZdrug,Age,Race,Sex,
                Income,Education,Marital,PYRMICat,MetroStatus) %>%
  mutate("PYRZdrug" = factor(PYRZdrug, levels = 0:1, labels = 2:1))
# Split the data into test and train datasets and create cross validation folds
set.seed(123)
zdrug.op2.split <- initial_split(zdrug.op2,strata=PYRZdrug)
zdrug.op2.train <- training(zdrug.op2.split)
zdrug.op2.test <- testing(zdrug.op2.split)
set.seed(321)
zdrug.op2.cv <- vfold_cv(zdrug.op2.train, v=10, repeats=5,strata=PYRZdrug)
# Generate preprocessing recipe
set.seed(111)
zdrug.op2.rec <- 
  recipe(PYRZdrug~Age+Race+Sex+Income+Education+Marital+PYRMICat+MetroStatus,zdrug.op2.train) %>%
  step_downsample(PYRZdrug) %>% # Downsample training data to match outcome class probability
  step_dummy(all_nominal_predictors()) # Convert factors to multiple dummy variables
# Create Workflow Set
set.seed(222)
zdrug.op2.wflow <- 
  workflow_set(
    preproc = list("downsample.impute" = zdrug.op2.rec),
    models = list("NB" = naivebayes.m, "Logistic" = logistic.m, "Tree" = decisiontree.m, "RF" = randomforest.m, "XGB" = xgboost.m, "SVM" = svm.m),
    cross = T
  )
# Set verbose printing for eliminating models in the tuning race
race_ctrl <- control_race(verbose_elim = T)
# Optimize hyperparameter tuning using the ANOVA racing method
zdrug.op2.race.results <- 
  zdrug.op2.wflow %>%
  workflow_map(
    fn = "tune_race_anova",
    seed = 312,
    resamples = zdrug.op2.cv,
    grid = 20,
    control = race_ctrl,
    verbose = T,
    metrics = metric_set(mcc,bal_accuracy,kap,j_index,f_meas,roc_auc)
...
# Review model results and pick best model based on MCC
zdrug.op2.rankresults <- rank_results(zdrug.op2.race.results, rank_metric = "mcc", select_best = T) %>% # Save table of model metrics data
  mutate("lower" = mean-std_err, "upper" = mean+std_err, 
         ".metric" = factor(.metric, levels = c("mcc","roc_auc","bal_accuracy","kap","j_index","f_meas"),
                            labels = c("MCC","ROC_AUC","Balanced Accuracy","Kappa","J Index","F Score")),
         "model" = factor(model, levels = c("logistic_reg","naive_Bayes","rand_forest","decision_tree","boost_tree","svm_rbf"),
                          labels = c("Regularized Logistic Regression","Naive Bayes","Random Forest","Decision Tree","Gradient Boosting","Support Vector Machine")))
zdrug.op2.bestmodel <- zdrug.op2.rankresults %>% filter(rank==1,.metric=="MCC") %>% dplyr::select(wflow_id) %>% flatten_chr() # Identify best model by MCC
zdrug.op2.modelplot <- ggscatter(filter(zdrug.op2.rankresults, .metric=="MCC" | .metric=="Balanced Accuracy" | .metric=="ROC_AUC"), x="model",y="mean",color="model") + 
  geom_errorbar(aes(ymin=lower, ymax=upper),width = 0.15) + facet_wrap(~.metric,scales="free",strip.position = "top")+
  ylab("Metric") + xlab("Z-drug use") + scale_x_discrete(labels = c("","","","","",""))+
  theme(legend.title = element_blank())
# Finalize model
zdrug.op2.best.results <- 
  zdrug.op2.race.results %>%
  pull_workflow_set_result("downsample.impute_Logistic") %>% 
  select_best(metric = "mcc")
zdrug.op2.final <- 
  zdrug.op2.race.results %>%
  pull_workflow("downsample.impute_Logistic") %>%
  finalize_workflow(zdrug.op2.best.results) %>%
  last_fit(split = zdrug.op2.split, metrics = metric_set(mcc, roc_auc, bal_accuracy))
# Collect Metrics and Predictions
zdrug.op2.final.metrics <- collect_metrics(zdrug.op2.final)
zdrug.op2.final.preds <- collect_predictions(zdrug.op2.final)
# Plot ROC Curve
zdrug.op2.roc <- autoplot(roc_curve(zdrug.op2.final.preds,truth = PYRZdrug,".pred_2"))
# Close unnecessary environment objects
rm(list = c("zdrug.op2", "zdrug.op2.cv", "zdrug.op2.rec", "zdrug.op2.test", "zdrug.op2.train", "zdrug.op2.split", "zdrug.op2.wflow"))
...
# Outcome 4: Sedative Benzodiazepines predicted by Covariates only
# Select data 
sedbenzo.op2 <- zdrug.data %>% 
  dplyr::select(SedBenzo,Age,Race,Sex,
                Income,Education,Marital,PYRMICat,MetroStatus)%>%
  mutate("SedBenzo" = factor(SedBenzo, levels = 0:1, labels = 2:1))
# Split the data into test and train datasets and create cross validation folds
set.seed(123)
sedbenzo.op2.split <- initial_split(sedbenzo.op2,strata=SedBenzo)
sedbenzo.op2.train <- training(sedbenzo.op2.split)
sedbenzo.op2.test <- testing(sedbenzo.op2.split)
set.seed(321)
sedbenzo.op2.cv <- vfold_cv(sedbenzo.op2.train, v=10, repeats=5,strata=SedBenzo)
# Generate preprocessing recipe
set.seed(111)
sedbenzo.op2.rec <- recipe(SedBenzo~Age+Race+Sex+Income+Education+Marital+PYRMICat+MetroStatus,sedbenzo.op2.train) %>%
  step_downsample(SedBenzo) %>%
  step_dummy(all_nominal_predictors())
# Create Workflow Set
set.seed(222)
sedbenzo.op2.wflow <- 
  workflow_set(
    preproc = list("downsample.impute" = sedbenzo.op2.rec),
    models = list("NB" = naivebayes.m, "Logistic" = logistic.m, "Tree" = decisiontree.m, "RF" = randomforest.m, "XGB" = xgboost.m, "SVM" = svm.m),
    cross = T
  )
# Tune model hyperparameters using ANOVA race
sedbenzo.op2.race.results <- 
  sedbenzo.op2.wflow %>%
  workflow_map(
    fn = "tune_race_anova",
    seed = 312,
    resamples = sedbenzo.op2.cv,
    grid = 20,
    control = race_ctrl,
    verbose = T,
    metrics = metric_set(mcc,bal_accuracy,kap,j_index,f_meas,roc_auc)
...
# Review model results and pick best model approach using MCC
sedbenzo.op2.rankresults <- rank_results(sedbenzo.op2.race.results, rank_metric = "mcc", select_best = T) %>% # Save table of model metrics data
  mutate("lower" = mean-std_err, "upper" = mean+std_err, 
         ".metric" = factor(.metric, levels = c("mcc","roc_auc","bal_accuracy","kap","j_index","f_meas"),
                            labels = c("MCC","ROC_AUC","Balanced Accuracy","Kappa","J Index","F Score")),
         "model" = factor(model, levels = c("logistic_reg","naive_Bayes","rand_forest","decision_tree","boost_tree","svm_rbf"),
                          labels = c("Regularized Logistic Regression","Naive Bayes","Random Forest","Decision Tree","Gradient Boosting","Support Vector Machine")))
sedbenzo.op2.bestmodel <- sedbenzo.op2.rankresults %>% filter(rank==1,.metric=="MCC") %>% dplyr::select(wflow_id) %>% flatten_chr()
sedbenzo.op2.modelplot <- ggscatter(filter(sedbenzo.op2.rankresults, .metric=="MCC" | .metric=="Balanced Accuracy" | .metric=="ROC_AUC"), x="model",y="mean",color="model") + 
  geom_errorbar(aes(ymin=lower, ymax=upper),width = 0.15) + facet_wrap(~.metric,scales="free",strip.position = "top")+
  ylab("Metric") + xlab("Sedative benzodiazepine use") + scale_x_discrete(labels = c("","","","","",""))+
  theme(legend.title = element_blank())
# Finalize model
sedbenzo.op2.best.results <- 
  sedbenzo.op2.race.results %>%
  pull_workflow_set_result("downsample.impute_Logistic") %>% 
  select_best(metric = "mcc")
sedbenzo.op2.final <- 
  sedbenzo.op2.race.results %>%
  pull_workflow("downsample.impute_Logistic") %>%
  finalize_workflow(sedbenzo.op2.best.results) %>%
  last_fit(split = sedbenzo.op2.split, metrics = metric_set(mcc, roc_auc, bal_accuracy))
# Collect Metrics and Predictions
sedbenzo.op2.final.metrics <- collect_metrics(sedbenzo.op2.final)
sedbenzo.op2.final.preds <- collect_predictions(sedbenzo.op2.final)
# Plot ROC Curve
sedbenzo.op2.roc <- autoplot(roc_curve(sedbenzo.op2.final.preds,truth = SedBenzo,".pred_2"))
## Close unnecessary environment objects
rm(list = c("sedbenzo.op2", "sedbenzo.op2.cv", "sedbenzo.op2.rec", "sedbenzo.op2.test", "sedbenzo.op2.train", "sedbenzo.op2.split", "sedbenzo.op2.wflow"))
stopCluster(cl)
...
# tiff("c:/users/atala/sync/research/projects/to publish/nsduh/drug use and z-drugs/fig1.tiff",width=2000, height=1800,res = 300)
# modelperf.p
# dev.off()

zdrug.p.data <- bind_rows(
  crossing("Model" = "With opioid use/misuse",
           roc_curve(zdrug.op.final.preds,truth = PYRZdrug,".pred_2")),
  crossing("Model" = "Without opioid use/misuse",
           roc_curve(zdrug.op2.final.preds,truth = PYRZdrug,".pred_2"))
) %>% mutate(
  Model = factor(Model, levels = c("With opioid use/misuse","Without opioid use/misuse"))
)
zdrug.p.roc <- ggplot(zdrug.p.data,aes(x=1-specificity,y=sensitivity,linetype=Model))+
  geom_path()+
  geom_abline(lty=3)+
  coord_equal()+
  theme_pubr()+ggtitle("Z-drug use")

sedbenzo.p.data <- bind_rows(
  crossing("Model" = "With opioid use/misuse",
           roc_curve(sedbenzo.op.final.preds,truth = SedBenzo,".pred_2")),
  crossing("Model" = "Without opioid use/misuse",
           roc_curve(sedbenzo.op2.final.preds,truth = SedBenzo,".pred_2"))
) %>% mutate(
  Model = factor(Model, levels = c("With opioid use/misuse","Without opioid use/misuse"))
)
sedbenzo.p.roc <- ggplot(sedbenzo.p.data,aes(x=1-specificity,y=sensitivity,linetype=Model))+
  geom_path()+
  geom_abline(lty=3)+
  coord_equal()+
  theme_pubr()+ggtitle("Sedative benzodiazepine use")

modelroc.p <- ggarrange(zdrug.p.roc,sedbenzo.p.roc,nrow=1,ncol=2,common.legend = T)

# tiff("c:/users/atala/sync/research/projects/to publish/nsduh/drug use and z-drugs/fig2.tiff",width=2000, height=1200,res = 300)
# modelroc.p
# dev.off()
